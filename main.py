#main.py
"""
Main script for the Target Audience Refinement Tool using COM-B and HPEM Models.
"""
...
area_of_focus = os.getenv("AREA_OF_FOCUS", "") # Area of focus
operational_objective = os.getenv("OPERATIONAL_OBJECTIVE", "") # Operational objective
constraints = os.getenv("CONSTRAINTS", "None") # Constraints
restraints = os.getenv("RESTRAINTS", "None") # Restraints
psychological_objective = os.getenv("PSYCHOLOGICAL_OBJECTIVE", "") # Psychological objective
spo = os.getenv("SPO", "") # Supporting Psychological Objective

def main():
    """
    Main function to run the program from multiple files.
    """
    ...
    """
    Define and refine the desired behavior using the influence_mission function.
    """
    ...
    print(f"Refined desired behavior: {refined_behavior}") # Refined desired behavior

    # Step 2: Break down the desired behavior into supporting behaviors
    supporting_behaviors = break_down_behavior(refined_behavior, area_of_focus)
    print("\n####################\n")
    # Fixing the formatting
    formatted_supporting_behaviors = "\n".join([f"{idx+1}. {behavior.strip()}" for idx, behavior in enumerate(supporting_behaviors) if behavior.strip()])
    print("####################")
    print("supporting behaviors:\n" + formatted_supporting_behaviors)
    print("\n####################\n")
    # Step 3: Describe the Potential Target Audience (PTA)
    pta_description = select_potential_target_audience(refined_behavior, supporting_behaviors, constraints, restraints, operational_context)
    print(f"Potential Target Audience (PTA): {pta_description}")
    print("\n####################\n")

    # Step 4: Assess Capability
    capable_pta = assess_capability(supporting_behaviors, refined_behavior, pta_description, area_of_focus, constraints, restraints, operational_context)
    print("\n####################\n")

    # Step 5: Assess Opportunity
    capable_opportune_pta = assess_opportunity(supporting_behaviors, capable_pta, area_of_focus, constraints, restraints, operational_context)
    print("\n####################\n")

    # Step 6: Assess Motivation
    refined_target_audience = assess_motivation(supporting_behaviors, capable_opportune_pta, area_of_focus, constraints, restraints, operational_context)
    print("\n####################\n")
